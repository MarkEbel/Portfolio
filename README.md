...
- **Renovate** — weekly grouped npm updates for `portfolio-website` (`renovate.json`). PRs (including majors) automerge with squash after required checks on `main` pass. TypeScript major updates are ignored: TypeScript 7 ships no programmatic API, so `typescript-eslint` caps its peer range at `<6.1.0` and `npm ci` cannot resolve the tree. Keep TypeScript on 6.x until `typescript-eslint` supports TypeScript 7.1's new API. Install the [Renovate GitHub App](https://github.com/apps/renovate) on this repo; do not require pull request reviews on `main`, or automerge will wait forever.
